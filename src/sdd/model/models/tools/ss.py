...
def image_scharr_gradients(img: "BCHW") -> "BC2HW":
    flipped_scharr_x = torch.tensor(
        [[-3, 0, 3], [-10, 0, 10], [-3, 0, 3]], dtype=img.dtype, device=img.device
    )
    kernel = torch.stack([flipped_scharr_x, flipped_scharr_x.t()]).unsqueeze(1)
    return F.conv2d(img.flatten(end_dim=-3).unsqueeze(1), kernel, padding=1).unflatten(
        0, img.shape[:-2]
    )
...
def image_gaussian_grads(img: "BCHW"):
    ...
    startx, starty = int(max(0, (xywh[-2] - img_width) / 2)), int(
        max(0, (xywh[-1] - img_height) / 2)
    )
    img_rotated = TF.rotate(img, 45.0, expand=True)
    grads_rotated = image_scharr_gradients(img_rotated)
    grads_rotated = TF.rotate(
        grads_rotated.flatten(end_dim=-3), -45.0, expand=True
    ).unflatten(0, grads_rotated.shape[:-2])
    grads_rotated = grads_rotated[
        ..., starty : starty + img_height, startx : startx + img_width
    ]
    ...
    return torch.cat(
        [
            grads.clamp(min=0),
            grads.clamp(max=0),
            grads_rotated.clamp(min=0),
            grads_rotated.clamp(max=0),
        ],
        dim=-3,
    )
...
def rotated_xywh(img_height: int, img_width: int, angle=45.0, scale=1.0):
    ...
    alpha, beta = scale * math.cos(math.radians(angle)), scale * math.sin(
        math.radians(angle)
    )
    rot = torch.tensor(
        [
    ...
    rotate = lambda point: (
        rot @ torch.tensor((point[0], point[1], 1.0), dtype=torch.float32)
    ).tolist()[:2]
...
def bbox_merge_tensor(
    xywh1: torch.Tensor, xywh2: torch.Tensor, dtype=None, return_area=False
) -> torch.Tensor:
    assert return_area
...
def bbox_merge_tuple(xywh1: tuple, xywh2: tuple, return_area=False):
    ...
    xmax, ymax = max(xywh1[0] + xywh1[2] - 1, xywh2[0] + xywh2[2] - 1), max(
        xywh1[1] + xywh1[3] - 1, xywh2[1] + xywh2[3] - 1
    )
...
def bbox_per_segment(reg_lab_int64: "BHW", max_num_segments: int, dtype=torch.int16):
    ...
    # HxW, HxW
    y, x = torch.meshgrid(
        torch.arange(img_height, dtype=dtype, device=reg_lab_int64.device),
        torch.arange(img_width, dtype=dtype, device=reg_lab_int64.device),
        indexing="ij",
    )
    y, x = y.expand_as(reg_lab_int64).flatten(start_dim=-2), x.expand_as(
        reg_lab_int64
    ).flatten(start_dim=-2)
    ...
    xywh = torch.zeros(
        4,
        *reg_lab_int64.shape[:-1],
        max_num_segments,
        dtype=dtype,
        device=reg_lab_int64.device
    )
...
def area_per_segment(reg_lab_int64: "BHW", max_num_segments: int, dtype=torch.int32):
    ...
    # BxS
    return torch.zeros(
        *reg_lab_int64.shape[:-2], max_num_segments, dtype=dtype
    ).scatter_add_(-1, I, expand_ones_like(I, dtype=dtype))
...
class SelectiveSearch(torch.nn.Module):
    def __init__(
        ...
        color_hist_bins=8 * 4,
        texture_hist_bins=8,
        remove_duplicate_boxes=False,
        randomize_rank=True,
        return_region_features=False,
    ):
        super().__init__()
        self.base_k = base_k
        self.inc_k = inc_k
        ...
        self.postprocess_labels = (
            ...
            else (lambda reg_lab: reg_lab)
        )

        if self.preset == "single":  # base_k = 200
            self.stack_color_spaces = lambda rgb, hsv, lab, gray: torch.stack(
                [hsv], dim=-4
            )
            self.segmentations = [
                cv2.ximgproc.segmentation.createGraphSegmentation(
                    self.sigma, float(base_k), self.min_size
                )
            ]
            self.strategies = [
                [0.25, 0.25, 0.25, 0.25],
            ...
            self.segmentations = [
                cv2.ximgproc.segmentation.createGraphSegmentation(
                    self.sigma, float(k), self.min_size
                )
                for k in range(
                    self.base_k, 1 + self.base_k + self.inc_k * 2, self.inc_k
                )
            ]
            self.strategies = [
                [0.25, 0.25, 0.25, 0.25],
            ...
            self.segmentations = [
                ...
                for k in range(
                    self.base_k, 1 + self.base_k + self.inc_k * 4, self.inc_k
                )
            ]
            self.strategies = [
                [0.25, 0.25, 0.25, 0.25],
    ...
    def get_region_mask(reg_lab, regs):
        return torch.stack(
            [
                (
                    reg_lab[reg["plane_id"][:-1]].unsqueeze(-1)
                    == torch.tensor(
                        list(reg["ids"]), device=reg_lab.device, dtype=reg_lab.dtype
                    )
                ).any(dim=-1)
                for reg in regs
            ]
    ...
    def forward(self, *, img_rgbb3hw_1: "B3HW", generator=None, print=print):
        ...
        assert (
            img_rgbb3hw_1.is_contiguous()
            and img_rgbb3hw_1.is_floating_point()
            and img_rgbb3hw_1.ndim == 4
            and img_rgbb3hw_1.shape[-3] == 3
        )
        ...
        hsv, lab, gray = (
            rgb_to_hsv(img_rgbb3hw_1),
            rgb_to_lab(img_rgbb3hw_1),
            rgb_to_grayscale(img_rgbb3hw_1),
        )
        ...
        grads = normalize_min_max_(
            image_gaussian_grads(imgs.flatten(end_dim=-4)).unflatten(
                0, imgs.shape[:-3]
            ),
            dim=(-2, -1),
        )
        print("image feat", time.time() - tic)
        tic = time.time()
        ...
        reg_lab = self.postprocess_labels(
            torch.stack(
                [
                    torch.as_tensor(gs.processImage(img))
                    for img in imgs.flatten(end_dim=-4)
                    .movedim(-3, -1)
                    .mul(255.0)
                    .contiguous()
                    .numpy()
        ...
        # (BCG)xSxS
        graphadj = self.build_graph(reg_lab, max_num_segments=max_num_segments).flatten(
            end_dim=-3
        )
        # (BCG)xSxSxT (T = # sTrategies)
        graphadj = (
            graphadj[..., None, None]
            * affinity.unsqueeze(-2)
            * torch.tensor(
                self.strategies, dtype=affinity.dtype, device=affinity.device
            )
        ).sum(dim=-1)
        ...
        PQ = list(
            zip(
                sim.neg().tolist(),
                (plane_idx * max_num_segments + r1).tolist(),
                (plane_idx * max_num_segments + r2).tolist(),
            )
        )
        # (BCGT)xSxS
        ...
        regs = [
            dict(
                ...
                idx=r1,
                parent_idx=-1,
                level=1 if r1 < int(num_segments[b, c, g]) else -1,
                bbox_xywh=tuple(
                    region_features.bbox_xywh_[plane_idx * max_num_segments + r1]
                ),
                region_size=region_features.region_size_[
                    plane_idx * max_num_segments + r1
                ],
                color_hist=region_features.color_hist_[
                    plane_idx * max_num_segments + r1
                ].clone()
                if self.return_region_features
                else None,
                texture_hist=region_features.texture_hist_[
                    plane_idx * max_num_segments + r1
                ].clone()
                if self.return_region_features
                else None,
            )
            for plane_idx, (b, c, g, t, strategy) in enumerate(
                (b, c, g, t, strategy)
                for b in range(num_segments.shape[0])
                for c in range(num_segments.shape[1])
                for g in range(num_segments.shape[2])
                for t, strategy in enumerate(self.strategies)
            )
            for r1 in range(graphadj.shape[-2])
        ]

        region_features.merge_regions_counter, region_features.merge_regions_time = (
            0,
            0.0,
        )
        (
            region_features.compute_region_affinity_counter,
            region_features.compute_region_affinity_time,
        ) = (0, 0.00)
        heapq.heapify(PQ)
        while PQ:
            ...
            regs.append(
                dict(
                    ...
                    plane_id=reg_fro["plane_id"],
                    plane_idx=reg_fro["plane_idx"],
                    id=min(reg_fro["id"], reg_to["id"]),
                    ids=reg_fro["ids"] | reg_to["ids"],
                    idx=len(regs),
                    parent_idx=-1,
                    level=1 + max(reg_fro["level"], reg_to["level"]),
                )
            )
            reg_fro["parent_idx"] = reg_to["parent_idx"] = len(regs) - 1
            tic = time.time()
            merged: dict = region_features.merge_regions_(
                reg_fro["id"], reg_to["id"], reg_fro["plane_idx"]
            )
            regs[-1]["bbox_xywh"] = merged["bbox_xywh"]
            regs[-1]["region_size"] = merged["region_size"]
            regs[-1]["texture_hist"] = (
                merged["texture_hist"].clone() if self.return_region_features else None
            )
            regs[-1]["color_hist"] = (
                merged["color_hist"].clone() if self.return_region_features else None
            )
            region_features.merge_regions_time += time.time() - tic
            region_features.merge_regions_counter += 1

            for new_edge in self.contract_graph_edge(
                region_features, u, v, reg_fro["parent_idx"], regs, graph
            ):
                heapq.heappush(PQ, new_edge)

        print(
            "merge reg",
            region_features.merge_regions_counter,
            region_features.merge_regions_time,
        )
        print(
            "sim reg",
            region_features.compute_region_affinity_counter,
        ...
        for reg, level_multiplier in zip(
            regs,
            torch.rand(len(regs), generator=generator).tolist()
            if self.randomize_rank
            else torch.ones(len(regs)).tolist(),
        ):
            reg["rank"] = reg["level"] * level_multiplier

        key_img_id, key_rank = (lambda reg: reg["plane_id"][0]), (
            lambda reg: reg["rank"]
        )
        regions_by_image = list(
            map(
                {
                    k: sorted(list(g), key=key_rank)
                    for k, g in itertools.groupby(
                        sorted(
                            [reg for reg in regs if reg["level"] >= 0], key=key_img_id
                        ),
                        key=key_img_id,
                    )
                }.get,
                range(len(img_rgbb3hw_1)),
            )
        )

        if self.remove_duplicate_boxes:
            boxes_xywh_without_duplicates = [
                {reg["bbox_xywh"]: i for i, reg in enumerate(by_image[b])}
                for b in range(len(img_rgbb3hw_1))
            ]
            boxes_xywh = [
                torch.tensor(
                    list(boxes_xywh_without_duplicates[b].keys()), dtype=torch.int16
                )
                for b in range(len(img_rgbb3hw_1))
            ]
            regions = [
                [by_image[b][i] for i in boxes_xywh_without_duplicates[b].values()]
                for b in range(len(img_rgbb3hw_1))
            ]
        else:
            boxes_xywh = [
                torch.tensor([reg["bbox_xywh"] for reg in regs], dtype=torch.int16)
                for regs in regions_by_image
            ]
    ...
    def build_graph(reg_lab: "BIGHW", max_num_segments: int):
        I = torch.stack(
            torch.meshgrid(
                *[torch.arange(s) for s in reg_lab.shape[:3]], indexing="ij"
            ),
            dim=-1,
        )[..., None, None]
        DX = torch.stack([reg_lab[..., :, :-1], reg_lab[..., :, 1:]], dim=-1).movedim(
            -1, -3
        )
        DY = torch.stack([reg_lab[..., :-1, :], reg_lab[..., 1:, :]], dim=-1).movedim(
            -1, -3
        )
        dx = torch.cat([I.expand(-1, -1, -1, -1, *DX.shape[-2:]), DX], dim=-3).flatten(
            start_dim=-2
        )
        dy = torch.cat([I.expand(-1, -1, -1, -1, *DY.shape[-2:]), DY], dim=-3).flatten(
            start_dim=-2
        )
        ...
        A = torch.sparse_coo_tensor(
            i,
            v,
            reg_lab.shape[:-2] + (max_num_segments, max_num_segments),
            dtype=torch.int64,
        )
        A += A.transpose(-1, -2)
    ...
    def contract_graph_edge(self, u, v, ww, regs, graph):
        ...
        for uu in [u, v]:
            for vv in graph.pop(uu, []):
                ...
                self.compute_region_affinity_time += time.time() - tic
                self.compute_region_affinity_counter += 1
...
class HandcraftedRegionFeatures:
    def __init__(
        ...
        # BxCxGxS
        self.region_size = area_per_segment(
            reg_lab_int64, max_num_segments=self.max_num_segments, dtype=torch.int32
        )
        # BxCxGxSx4
        self.bbox_xywh = bbox_per_segment(
            reg_lab_int64, max_num_segments=self.max_num_segments, dtype=torch.int16
        )

        # BxCxGx3x(HW), img_bins is broadcasted
        I = (reg_lab[..., None, :, :] * color_hist_bins + imgs_bins).flatten(
            start_dim=-2
        )
        ...
        self.color_hist = (
            torch.zeros(
                *reg_lab.shape[:-2],
                img_channels,
                self.max_num_segments * color_hist_bins,
                dtype=torch.float32
            )
            .scatter_add_(
                -1, I.to(torch.int64), expand_ones_like(I, dtype=torch.float32)
            )
            .unflatten(-1, (self.max_num_segments, color_hist_bins))
            .movedim(-2, -3)
            .flatten(start_dim=-2)
        )
        # for histogram normalization, each pixel contributes to 3 = #colorchannels bins, eps to avoid divbyzero and nans in affinity
        self.color_hist /= (
            self.region_size.mul(float(imgs_bins.shape[-3])).add_(eps).unsqueeze(-1)
        )

        # BxCxGx3xRx(HW), grads_bins is broadcasted
        I = (reg_lab[..., None, None, :, :] * texture_hist_bins + grads_bins).flatten(
            start_dim=-2
        )
        ...
        self.texture_hist = (
            torch.zeros(
                *reg_lab.shape[:-2],
                img_channels,
                grads_bins.shape[-3],
                self.max_num_segments * texture_hist_bins,
                dtype=torch.float32
            )
            .scatter_add_(
                -1, I.to(torch.int64), expand_ones_like(I, dtype=torch.float32)
            )
            .unflatten(-1, (self.max_num_segments, texture_hist_bins))
            .movedim(-2, -4)
            .flatten(start_dim=-3)
        )
        ...
        self.texture_hist /= (
            self.region_size.mul(float(grads_bins.shape[-4] * grads_bins.shape[-3]))
            .add_(eps)
            .unsqueeze(-1)
        )
        #
        self.buf = torch.empty(
            max(self.texture_hist.shape[-1], self.color_hist.shape[-1]),
            dtype=torch.float32,
        )

    def repeat_for_strategies_and_flatten(self, num_strategies):
        self.region_size_ = (
            self.region_size.flatten(end_dim=-2)
            .unsqueeze(-2)
            .repeat(1, num_strategies, 1)
            .flatten()
            .tolist()
        )
        self.bbox_xywh_ = (
            self.bbox_xywh.flatten(end_dim=-3)
            .unsqueeze(-3)
            .repeat(1, num_strategies, 1, 1)
            .flatten(end_dim=-2)
            .tolist()
        )
        self.color_hist_ = (
            self.color_hist.flatten(end_dim=-3)
            .unsqueeze(-3)
            .repeat(1, num_strategies, 1, 1)
            .flatten(end_dim=-2)
        )
        self.texture_hist_ = (
            self.texture_hist.flatten(end_dim=-3)
            .unsqueeze(-3)
            .repeat(1, num_strategies, 1, 1)
            .flatten(end_dim=-2)
        )
    ...
    def merge_regions_(self, r1, r2, plane_idx) -> dict:
        ...
        self.region_size_[plane_idx * self.max_num_segments + r2] = self.region_size_[
            plane_idx * self.max_num_segments + r1
        ] = s1s2
        self.bbox_xywh_[plane_idx * self.max_num_segments + r2] = self.bbox_xywh_[
            plane_idx * self.max_num_segments + r1
        ] = bbox_merge_tuple(
            self.bbox_xywh_[plane_idx * self.max_num_segments + r1],
            self.bbox_xywh_[plane_idx * self.max_num_segments + r2],
        ...
        self.color_hist_[plane_idx * self.max_num_segments + r2].copy_(
            self.color_hist_[plane_idx * self.max_num_segments + r1]
            .mul_(s1)
            .add_(self.color_hist_[plane_idx * self.max_num_segments + r2].mul_(s2))
            .div_(s1s2)
        )
        self.texture_hist_[plane_idx * self.max_num_segments + r2].copy_(
            self.texture_hist_[plane_idx * self.max_num_segments + r1]
            .mul_(s1)
            .add_(self.texture_hist_[plane_idx * self.max_num_segments + r2].mul_(s2))
            .div_(s1s2)
        )
    ...
    @staticmethod
    def compute_region_affinity_pdist(
        img_size, region_size, bbox_xywh, color_hist, texture_hist
    ):
        size_affinity = (
            region_size.unsqueeze(-1)
            .add(region_size.unsqueeze(-2))
            .div(-float(img_size))
            .add_(1)
            .clamp_(min=0, max=1)
        )
        fill_affinity = (
            bbox_merge_tensor(
                bbox_xywh.unsqueeze(-2),
                bbox_xywh.unsqueeze(-3),
                dtype=region_size.dtype,
                return_area=True,
            )
            .sub_(region_size.unsqueeze(-1))
            .sub_(region_size.unsqueeze(-2))
            .div(-float(img_size))
            .add_(1)
            .clamp_(min=0, max=1)
        )
        color_affinity = torch.min(
            color_hist.unsqueeze(-2), color_hist.unsqueeze(-3)
        ).sum(dim=-1)
        texture_affinity = torch.min(
            texture_hist.unsqueeze(-2), texture_hist.unsqueeze(-3)
        ).sum(dim=-1)
        return torch.stack(
            [fill_affinity, texture_affinity, size_affinity, color_affinity], dim=-1
        )

    @staticmethod
    def compute_region_affinity_pairwise_distance(
        img_size,
        region_size_1,
        bbox_xywh_1,
        color_hist_1,
        ...
        size = (
            0.0
            if size == 0
            else size * max(0, min(1, 1 - (region_size_1 + region_size_2) / img_size))
        )
        fill = (
            0.0
            if fill == 0
            ...
            * max(
                ...
                min(
                    ...
                    1
                    - (
                        bbox_merge_tuple(bbox_xywh_1, bbox_xywh_2, return_area=True)
                        - region_size_1
                        - region_size_2
                    )
                    / img_size,
        ...
        color = (
            ...
            * float(
                torch.min(
                    color_hist_1,
                    color_hist_2,
                    out=None if buf is None else buf.resize_(0),
                ).sum(dim=-1)
            )
        )
        texture = (
